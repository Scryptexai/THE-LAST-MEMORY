...
├── assets/art/portraits/         # potret anime tokoh (<id>.png 448×672) + varian ekspresi <id>_{happy,sad,surprised,angry}.png (384×576)
...
- **Tokoh sprite** (`Actor2D`/`NPC2D`/`Player2D`): PNG seluruh badan `assets/art/sprites/<id>.png` (8/8: Ardi, Rara, Pak Harto, Mira, Nenek, Darmo, Bu RT, Warga — id `warga` dipakai Pedagang/Penjaga/Juru Kunci; siluet prosedural hanya fallback bila berkas hilang), titik asal di kaki, skala dari kedalaman, **napas**, **goyang jalan**, **balik arah**, **lompatan sapaan** saat Ardi mendekat, **redup** saat bukan pembicara, bayangan lembut, cincin sorot saat hover. NPC dengan `wander` berjalan santai di sekitar posnya.
...
- **Potret dialog** `assets/art/portraits/<id>.png` via `ThemeFactory.portrait(nama, emosi)` (alias nama pembicara → id), sepia saat kilas balik. **Ekspresi**: simpul dialog boleh memuat `"emotion": "happy" | "sad" | "surprised" | "angry"` (98 simpul sudah ditandai) → potret berganti ke `<id>_<emosi>.png` dengan kedip lembut; tokoh tanpa varian (Warga) otomatis kembali ke potret netral. Smoke test memastikan setiap emosi yang dipakai punya berkasnya; tab Tokoh jurnal memakai potret yang sama. **Key art** `assets/art/ui/keyart_menu.png` di menu utama (Ken Burns pelan). `ThemeFactory.art_texture()` memuat PNG/JPG langsung bila `.import` belum ada (build headless).
